...
///
/// Remaining unit notes vs. Python SystemTracker:
///   cpu_usage        - fractional cores (0..N), same as Python
///   memory_*         - kibibytes (KiB), same as Python
///   disk_read/write  - cumulative bytes since boot, same as Python
///   net_recv/sent    - cumulative bytes since boot, same as Python
...
pub fn sample_to_csv_row(s: &Sample, interval_secs: u64) -> String {
    // cpu_usage: fractional cores (0..N), matching Python's convention
    let cpu_usage = s.cpu.utilization_pct / 100.0 * s.cpu.total_cores as f64;
    ...
    let (disk_total_gb, disk_used_gb, disk_free_gb) =
        (disk_space_total, disk_space_used, disk_space_free);
    ...
    let gpu_utilized: u32 = s.gpu.iter().filter(|g| g.utilization_pct > 0.0).count() as u32;
    ...
    format!(
        ...
        s.memory.free_kib,
        s.memory.used_kib,
        s.memory.buffers_kib,
        s.memory.cached_kib,
        s.memory.active_kib,
        s.memory.inactive_kib,
        disk_read,
        disk_write,
        disk_total_gb,
        disk_used_gb,
