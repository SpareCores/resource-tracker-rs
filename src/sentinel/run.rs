...
fn base64_encode(input: &[u8]) -> String {
    ...
    for chunk in input.chunks(3) {
        ...
        out.push(if chunk.len() > 1 { char::from(ALPHA[((n >>  6) & 0x3f) as usize]) } else { '=' });
        out.push(if chunk.len() > 2 { char::from(ALPHA[( n        & 0x3f) as usize]) } else { '=' });
    }
    out
}
...
fn days_since_epoch(y: u64, mo: u64, d: u64) -> Option<u64> {
    ...
    let year_days: u64 = (1970..y).map(|yr| if (yr % 4 == 0 && yr % 100 != 0) || yr % 400 == 0 { 366 } else { 365 }).sum();
    let month_days: u64 = (1..mo).map(|m| {
        if m == 2 && is_leap { 29 } else { DAYS[(m - 1) as usize] }
    }).sum();
...
enum DataSource {
    S3     { data_uris: Vec<String> },
    /// Inline gzip-compressed, base64-encoded CSV.
    /// Matches Python `DataSource.inline` / `finish_run(data_source="inline", data_csv=...)`.
    Inline { data_csv:  String      },
}
...
/// local stdout output continues normally (§11 error handling).
pub fn start_run(
    agent:    &ureq::Agent,
    api_base: &str,
    token:    &str,
    ...
    let resp: StartRunResponse = serde_json::from_str(&text)
        .map_err(|e| format!("start_run parse response failed: {e} ({} bytes)", text.len()))?;
    ...
    Ok(RunContext {
        ...
        credentials: UploadCredentials {
            ...
            expires_at:        resp.upload_credentials.expiration
...
pub fn refresh_credentials(
    ...
    let resp: RefreshCredentialsResponse = serde_json::from_str(&text)
        .map_err(|e| format!("credential refresh parse failed: {e} ({} bytes)", text.len()))?;
...
/// `remaining_csv` is gzip-compressed and base64-encoded, then sent as
/// `data_source = "inline"` (matching Python `finish_run` behaviour).
pub fn close_run(
    agent:         &ureq::Agent,
    api_base:      &str,
    token:         &str,
    ...
    let data = if !uploaded_uris.is_empty() {
        DataSource::S3 { data_uris: uploaded_uris.to_vec() }
    } else {
        // Gzip-compress then base64-encode, matching Python:
        //   payload["data_csv"] = b64encode(data_csv).decode("ascii")
        let encoded = remaining_csv
            .map(|csv| {
                super::upload::gzip_compress(csv.as_bytes())
                    .map(|gz| base64_encode(&gz))
                    .unwrap_or_default()
            })
            .unwrap_or_default();
...
mod tests {
    ...
    #[test]
    fn creds_expiring_soon_far_future() {
        let ctx = RunContext {
    ...
    #[test]
    fn creds_expiring_soon_unparseable() {
        let ctx = RunContext {
            run_id:            "test".to_string(),
            upload_uri_prefix: "s3://b/p".to_string(),
            credentials: UploadCredentials {
    ...
    #[test]
    fn parse_iso8601_secs_known() {
        // 2026-04-01T00:00:00Z = 20544 days * 86400
        assert_eq!(parse_iso8601_secs("2026-04-01T00:00:00Z"), Some(20544 * 86400));
    }
    ...
    #[test]
    fn close_run_request_contains_run_id() {
        let req = CloseRunRequest {
            run_id:     "run-abc-123".to_string(),
            exit_code:  Some(0),
            run_status: "finished",
            data:       DataSource::Inline { data_csv: "aGVhZGVyCnJvdwo=".to_string() },
        };
    ...
    #[test]
    fn close_run_data_source_inline_when_no_uploads() {
        let data = DataSource::Inline { data_csv: "base64encodedgzip==".to_string() };
    ...
    #[test]
    fn close_run_data_source_s3_when_uploads_present() {
